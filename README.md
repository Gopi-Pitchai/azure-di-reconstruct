...
`azure-di-reconstruct` takes the JSON returned by the [Azure Document Intelligence](https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/) `prebuilt-read` model and reproduces the original document's two-dimensional layout as a monospace text grid â€” no image file, no external dependencies.
